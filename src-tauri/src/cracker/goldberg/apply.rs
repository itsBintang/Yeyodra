...
use crate::cracker::goldberg::{depots, dlcs, languages};
use dirs::data_dir;
use log::info;
use serde_json::json;
...
pub async fn apply_goldberg(
    ...
    for (dll_path, is_64bit) in found_dlls {
        ...
        // Fetch achievements, stats, and images (optional - continue if fails)
        match super::achievements::fetch_achievements(
            &app_handle,
            &app_id,
            language.clone(),
            &steam_settings_dir,
        ...
        {
            Ok(_) => {
                info!("✓ Successfully fetched achievements");
                app_handle.emit("crack-progress", &json!({"progress": current_progress as u32, "message": "Fetched achievements and images"})).ok();
            }
            Err(e) => {
                // CRITICAL: Create EMPTY achievement/stats files so game doesn't crash
                info!("⚠ Failed to fetch achievements (continuing anyway): {}", e);
                info!("Creating empty achievement/stats files to prevent crashes...");
                
                // Create empty achievements.json
                let achievements_path = steam_settings_dir.join("achievements.json");
                if let Err(write_err) = fs::write(&achievements_path, "[]") {
                    eprintln!("Failed to create empty achievements.json: {}", write_err);
                } else {
                    info!("Created empty achievements.json");
                }
                
                // Create empty stats.json
                let stats_path = steam_settings_dir.join("stats.json");
                if let Err(write_err) = fs::write(&stats_path, "[]") {
                    eprintln!("Failed to create empty stats.json: {}", write_err);
                } else {
                    info!("Created empty stats.json");
                }
                
                app_handle.emit("crack-progress", &json!({"progress": current_progress as u32, "message": "Created empty achievement files"})).ok();
            }
        }
        current_progress += substep;
        ...
        // Fetch and write DLCs (optional - continue if fails)
        match dlcs::fetch_and_write_dlcs(&app_id, &steam_settings_dir).await {
            Ok(_) => {
                info!("✓ Successfully fetched DLCs");
                app_handle.emit("crack-progress", &json!({"progress": current_progress as u32, "message": "Fetched DLCs"})).ok();
            }
            Err(e) => {
                // CRITICAL: Create EMPTY DLC.txt so game doesn't crash
                info!("⚠ Failed to fetch DLCs (continuing anyway): {}", e);
                info!("Creating empty DLC.txt to prevent crashes...");
                
                let dlc_path = steam_settings_dir.join("DLC.txt");
                if let Err(write_err) = fs::write(&dlc_path, "") {
                    eprintln!("Failed to create empty DLC.txt: {}", write_err);
                } else {
                    info!("Created empty DLC.txt");
                }
                
                app_handle.emit("crack-progress", &json!({"progress": current_progress as u32, "message": "Created empty DLC file"})).ok();
            }
        }
        current_progress += substep;

        // Fetch and write depots (optional - continue if fails)
        match depots::fetch_and_write_depots(&app_id, &steam_settings_dir).await {
            Ok(_) => {
                info!("✓ Successfully fetched depots");
                app_handle.emit("crack-progress", &json!({"progress": current_progress as u32, "message": "Fetched depots"})).ok();
            }
            Err(e) => {
                // CRITICAL: Create EMPTY depots.txt so game doesn't crash
                info!("⚠ Failed to fetch depots (continuing anyway): {}", e);
                info!("Creating empty depots.txt to prevent crashes...");
                
                let depots_path = steam_settings_dir.join("depots.txt");
                if let Err(write_err) = fs::write(&depots_path, "") {
                    eprintln!("Failed to create empty depots.txt: {}", write_err);
                } else {
                    info!("Created empty depots.txt");
                }
                
                app_handle.emit("crack-progress", &json!({"progress": current_progress as u32, "message": "Created empty depots file"})).ok();
            }
        }
        current_progress += substep;

        // Fetch and write supported languages (optional - continue if fails)
        match languages::fetch_and_write_languages(&app_id, &steam_settings_dir).await {
            Ok(_) => {
                info!("✓ Successfully fetched languages");
                app_handle.emit("crack-progress", &json!({"progress": current_progress as u32, "message": "Fetched languages"})).ok();
            }
            Err(e) => {
                // CRITICAL: Create EMPTY supported_languages.txt so game doesn't crash
                info!("⚠ Failed to fetch languages (continuing anyway): {}", e);
                info!("Creating empty supported_languages.txt with default 'english'...");
                
                let languages_path = steam_settings_dir.join("supported_languages.txt");
                // Write at least "english" as default language
                if let Err(write_err) = fs::write(&languages_path, "english\n") {
                    eprintln!("Failed to create empty supported_languages.txt: {}", write_err);
                } else {
                    info!("Created supported_languages.txt with 'english'");
                }
                
                app_handle.emit("crack-progress", &json!({"progress": current_progress as u32, "message": "Created default language file"})).ok();
            }
        }
        current_progress += substep;
